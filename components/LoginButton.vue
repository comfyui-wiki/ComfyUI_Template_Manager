<template>
  <div class="flex flex-col items-center gap-4">
    <div v-if="status === 'authenticated'" class="flex items-center gap-4">
      <div class="flex items-center gap-2">
        <img
          v-if="data?.user?.image"
          :src="data.user.image"
          :alt="data.user.name || 'User'"
          class="w-8 h-8 rounded-full"
        />
        <span class="text-sm font-medium">{{ data?.user?.name || data?.user?.email }}</span>
      </div>
      <Button @click="signOut" variant="outline" size="sm">
        Sign Out
      </Button>
    </div>

    <div v-else-if="modeReady && !isLocalClone" class="text-center">
      <Button @click="signIn('github')" size="lg" class="gap-2">
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
        </svg>
        Sign in with GitHub
      </Button>
      <p class="text-sm text-muted-foreground mt-2">
        Sign in to create and submit templates
      </p>
    </div>
  </div>
</template>

<script setup lang="ts">
import { computed, onMounted } from 'vue'
import { Button } from '@/components/ui/button'

const { status, data, signIn, signOut: authSignOut } = useAuth()
const { resetToMain, isLocalMode } = useGitHubRepo()
const { modeInfo, loadMode } = useRepoMode()
const { clearCache } = useTemplateDiff()

const modeReady = computed(() => modeInfo.value.loaded)
const isLocalClone = computed(() => isLocalMode.value || modeInfo.value.localRepoMode)

onMounted(() => {
  loadMode()
})

// Custom sign out handler that resets to main repo
const signOut = async () => {
  // Reset to main repo and branch (clears state and localStorage)
  resetToMain()
  console.log('[Sign Out] Reset to main repo and branch')

  // Clear all template caches
  clearCache()
  console.log('[Sign Out] Cleared template cache')

  // Call the actual auth sign out
  await authSignOut()

  // Force page reload to ensure all state is reset
  // This prevents any stale fork/branch data from persisting
  if (process.client) {
    console.log('[Sign Out] Reloading page to ensure clean state')
    window.location.href = '/'
  }
}
</script>
